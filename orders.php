...
<head>
  ...
  <title>Hero's Vault Shop : Orders</title>
  <!-- Bootstrap -->
...
<body>
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-9">
          <input name="orderdate" type="text" class="form-control" id="date" placeholder="Order Date" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>">
        </div>
      </div>  
    ...
    <div class="form-group">
        <label for="custid" class="col-sm-3 control-label">Customer</label>
        <div class="col-sm-9">
          <select name="cid" class="form-control" id="custid" required>
         <?php
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $stmt = $conn->prepare("SELECT * FROM tbl_customers_a202713_pt2");
            $stmt->execute();
            $result = $stmt->fetchAll();
          }
          catch(PDOException $e){
   ...
   <div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a202713, tbl_staffs_a202713_pt2, tbl_customers_a202713_pt2 WHERE ";
        $sql = $sql."tbl_orders_a202713.fld_staff_num = tbl_staffs_a202713_pt2.FLD_STAFF_ID and ";
        $sql = $sql."tbl_orders_a202713.fld_customer_num = tbl_customers_a202713_pt2.FLD_CUST_ID";
...
<div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
             $sql = "SELECT COUNT(*) FROM tbl_orders_a202713, tbl_staffs_a202713_pt2, tbl_customers_a202713_pt2 
          WHERE tbl_orders_a202713.fld_staff_num = tbl_staffs_a202713_pt2.FLD_STAFF_ID 
          AND tbl_orders_a202713.fld_customer_num = tbl_customers_a202713_pt2.FLD_CUST_ID";
