...
try {
  ...
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare("SELECT * FROM tbl_orders, tbl_staffs_pt2,
        tbl_customers_pt2, tbl_orders_details WHERE
        tbl_orders.fld_staff_num = tbl_staffs_pt2.FLD_STAFF_ID AND
        tbl_orders.fld_customer_num = tbl_customers_pt2.FLD_CUST_ID AND
  ...
  <div class="col-xs-5">
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4>From: Hero's Vault Sdn. Bhd.</h4>
      </div>
  ...
  try {
    ...
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $stmt = $conn->prepare("SELECT * FROM tbl_orders_details,
        tbl_products_pt2 where 
        tbl_orders_details.fld_product_num = tbl_products_pt2.FLD_PRODUCT_ID AND
        fld_order_num = :oid");
    $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
      $oid = $_GET['oid'];
    $stmt->execute();
