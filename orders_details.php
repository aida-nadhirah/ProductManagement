...
<head>
  ...
  <title>Hero's Vault Shop : Order Details</title>
  <!-- Bootstrap -->
...
<?php include_once 'nav_bar.php'; ?>
    ...
    try {
      ...
      $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $stmt = $conn->prepare("SELECT * FROM tbl_orders_a202713, tbl_staffs_a202713_pt2,
          tbl_customers_a202713_pt2 WHERE
          tbl_orders_a202713.fld_staff_num = tbl_staffs_a202713_pt2.FLD_STAFF_ID AND
          tbl_orders_a202713.fld_customer_num = tbl_customers_a202713_pt2.FLD_CUST_ID AND
          fld_order_num = :oid");
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a202713,
            tbl_products_a202713_pt2 WHERE
            tbl_orders_details_a202713.fld_product_num = tbl_products_a202713_pt2.FLD_PRODUCT_ID AND
          fld_order_num = :oid");
          $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
          $oid = $_GET['oid'];
        $stmt->execute();
